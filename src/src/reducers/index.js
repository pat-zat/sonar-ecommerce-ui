...
import * as actionNames from '../actions/names.js';
import createState from '../reduxUtils/createState';

function mergeSortDescriptors(sourceDescriptors, newDesciptor) {
  let result = [];
...
export function mergeFilters(sourceFilters, newFilter) {
  let result = { logic: 'and', filters: [] };
  ...
  if (sourceFilters.length || (sourceFilters.filters && sourceFilters.filters.length && newFilter)) {
    ...
    sourceFilters.filters.forEach((currentFilter) => {
      ...
      if (currentFilter.field === newFilter.filters[0].field) {
        let tmp = { logic: newFilter.logic, filters: [] };
